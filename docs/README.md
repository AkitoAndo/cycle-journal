...
日記・ジャーナルとAIコーチをつなげる内省アプリ。日記を書き、AIコーチとの対話を通じて自己理解を深め、小さな行動につなげる。

## コンセプト

Treowは「大きな一本の樹」をメタファーとしたコーチングアプリ。ユーザーが自分の感情や価値観を言葉にし、理解し、行動につなげていく循環を支援する。最終的にはアプリがなくても自分と向き合える力を育てることが目的。

## 主な機能

| 機能 | 概要 | 状態 |
|------|------|------|
| Journal | 日記・ジャーナルの作成・編集・タグ管理・検索 | iOS実装済 |
| Coach | AIコーチとの対話（Treowモデルベース） | API接続済 |
